...
function create_article_table()
{
	...
	ajax: "./get_articles.php",
        ...
        columns: [
            ...
            { title: "Ảnh", data:"feature_image", "searchable": false, className: "min-desktop"},
	    ...
	    data.sapo = data.sapo.replace(/"/g, "");
            $('td:eq(2)', row).html('<a href="' + data.href + '" target="_blank"' + 'onMouseEnter="display_sapo(this, ' + data.stt + ',`' + data.sapo + '`)">' + topic + '</a>');
	    $('td:eq(2)', row).on("tap",function(){
	    ...
	    // create image list
          ...
          if (data.feature_image.length > 0) {
            image_html = image_html + '<td>'
              + '<img src="' + data.feature_image[0]
              + '" style="height:160px; width:210px" onClick="display_image(`image' + data.stt + '`)"'
              + '/></td>'
          }
